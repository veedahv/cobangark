...
const withdrawBody = document.querySelector('#recent-withdraw-tbody');
...
const addWithdraws = () => {
    recentWithdraws.forEach(withdraw => {
        let withdrawRow = `
        <tr>
            <td class="px-8 py-5 text-sm">${withdraw.date}</td>
            <td class="px-8 py-5 text-sm">&dollar;${withdraw.amount}</td>
        </tr>       
        `

        withdrawBody.innerHTML += withdrawRow;
    });
}
